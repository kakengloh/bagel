...
describe('listen', () => {
  ...
  it('should parse path params', async () => {
    ...
    const response = await fetch('http://localhost:9003/path/hello');
    expect(response.status).toBe(200);
    const json = await response.json<Record<string, unknown>>();
  ...
  it('should parse query params', async () => {
    ...
    const response = await fetch('http://localhost:9004/?a=1&b=2');
    expect(response.status).toBe(200);
    const json = await response.json<Record<string, unknown>>();
  ...
  it('should return response with 500 status code', async () => {
    ...
    const response = await fetch('http://localhost:9006');
